...
Crie um banco de dados com o nome do seu projeto, lembre de utilizar um nome sem espaços ou caracteres especiais. Aqui vamos chamas nosso banco de `flor_e_cultura`.

```sql
CREATE DATABASE flor_e_cultura;
```
...
```sql
CREATE USER floriculturatcc2025 WITH ENCRYPTED PASSWORD 'tcc2025flower';
GRANT ALL ON SCHEMA public TO floriculturatcc2025;
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO floriculturatcc2025;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO floriculturatcc2025;
```

Para desconectar do terminal do PostgreSQL basta digitar o comando `exit` ou o atalho `CTRL`+`D`. Com seu banco criado para acessar ele diretamente basta utilizar o comando abaixo. Agora é possível logar diretamente no seu banco de dados `flor_e_cultura` no servidor `localhost` utilizando o usuário `floriculturatcc2025` diretamente da linha de comando fornecendo a senha.

```bash
psql -h localhost  -d flor_e_cultura -U floriculturatcc2025 -W
```
...
```bash
 psql -h localhost  -d flor_e_cultura -U floriculturatcc2025 -W -f db/schema_create.sql
```
