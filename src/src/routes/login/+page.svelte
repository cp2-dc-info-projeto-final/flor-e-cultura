  ...
  async function loginUsuario() {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Atualiza o store do usuário imediatamente após o login
        await refreshUser();
      ...
      <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
        ...
        <form on:submit|preventDefault={loginUsuario} class="space-y-4 md:space-y-6">
          ...
          <p class="text-sm font-light text-gray-500 dark:text-gray-400">
            É novo por aqui? Cadastre-se agora! <a href="/cadastro" class="font-medium text-green-600 hover:underline dark:text-green-400">Criar cadastro</a>
          </p>
        </form>
      </div>
    </div>
